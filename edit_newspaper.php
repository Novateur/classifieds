...
<?php echo "<body onload=\"get_newspaper_edit_form({$newspaper_id})\">"; ?>
            ...
            <section>
              <section class="vbox">
                <section class="scrollable padder"><br/>              
					<div class="col-sm-12">
                      <div class="alert alert-success"><h2 class="m-b-xs text-black">Edit newspaper</h2></div>
                    </div>
					<div class="col-sm-3">
					</div>					
					<div class="col-sm-6">
  ...
  <script>
	function get_newspaper_edit_form(id)
	{
		$.post("handler/get_newspaper_edit_form.php",{id:id},function(response){
			$('#paste_form').html(response);
		})
	}
	$('#edit_newspaper_form').submit(function(e){
		e.preventDefault();
		...
		else
		{
			document.getElementById("loading1").style.display="block";
				$.ajax({
					url:"handler/update_newspaper.php",
					type:"POST",
					...
					success:function(data)
					{
						...
						if(data=="updated")
						{
							...
							$('#response_msg').html("<div class='alert alert-danger'>Newspaper updated successfully</div>").slideDown().delay(3000).slideUp();
						}
						else
						{
							document.getElementById("loading1").style.display="none";
