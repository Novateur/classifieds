...
<?php echo "<body>"; ?>
							...
							<div class="col-sm-12">
								...
								<form id="add_section_form">
									...
									<div class="checkbox i-checks">
											<label>
												<input type="checkbox" name="mark_all" id="mark_all1" onchange="mark_action(this)">
												<i></i>
												Mark all
											</label>
									</div><hr/>
									<div style="max-height:300px;overflow:auto">
										<?php
											get_select_newspaper_section();
										?>
									</div>
									</select><br/>
									<label>Section Name<span class="req">*</span></label><br/>
  ...
  <script>
	$('#add_section_form').submit(function(e){
		...
		if(name=="")
		{
			if ($('.ma_newspaper:checked').length == 0) {
				alert("ffffff")
			}
			if(name=="")
			{
				...
				$.ajax({
					...
					success:function(data)
					{
						...
						if(data=="added")
						{
							...
							$('#response_msg').html("<div class='alert alert-info'>You have successfully added a section</div>").slideDown().delay(3000).slideUp();
						}
						else
						{
							document.getElementById("loading1").style.display="none";
	...
	/*function bbb()
	{
		if($('#mark_all1').prop('checked',true){
			
		}
		else
		{
			alert("hfjdkjkdjkdhgkgf");
		}
	}*/
	function mark_action(id)
	{
		if(id.checked) 
		{
