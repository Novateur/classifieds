<?php
	...
	$sql_pag="SELECT COUNT(*) FROM section WHERE newspaper='{$newspaper}'";
	...
	$sql = "SELECT * FROM section WHERE newspaper='{$newspaper}' LIMIT $start, $limit";
	...
	if($query->rowCount()>0)
	{
		...
		foreach($query as $r)
		{
			...
			<td>{$r['newspaper']}</td>
			<td><a href='#' data-role='button' class='btn btn-danger btn-sm' onclick=\"delete_newspaper_section({$r['id']},{$page},'{$newspaper}')\"><i class='i i-trashcan'></i></a></td>
