<?php
	...
	if($image=="")
	{
		$sql="UPDATE newspapers SET name_newspaper='{$name}',description='{$desc}' WHERE id='{$id}'";
	...
	else
	{
		...
		if($extension=="jpg" || $extension=="jpeg" || $extension=="png" && $size<=2000000)
		{
			...
			$sql="UPDATE newspapers SET name_newspaper='{$name}',description='{$desc}',logo='{$rename}' WHERE id='{$id}'";
			$query=$connection->query($sql);
			if($query)
			{
				echo "updated";
